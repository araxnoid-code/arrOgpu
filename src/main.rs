...
fn main() {
    ...
    let contiguous_a = array_a_view.contiguous_metadata().unwrap();
    ...
    let contiguous_b = array_b_view.contiguous_metadata().unwrap();
    // println!("{}", array_b_view.contiguous_metadata().unwrap());

    // let dot = array_a_view.dot(&array_b_view).unwrap();
    let dot = contiguous_a.dot(&contiguous_b).unwrap();
    println!("{}", dot);
