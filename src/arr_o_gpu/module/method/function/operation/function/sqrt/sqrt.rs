...
impl ArrOgpuModule {
    pub fn sqrt<'a, A>(&self, array: &'a A) -> Result<GpuArray, ArrOgpuErr>
    where
        ...
        let mut encoder =
            wgpu.device
                .create_command_encoder(&wgpu::wgt::CommandEncoderDescriptor {
                    label: Some("Create Encoder For Sqrt"),
                });
        ...
        crate::ArrayType::Contiguous(_) => {
            device.create_shader_module(wgpu::ShaderModuleDescriptor {
                ...
                source: wgpu::ShaderSource::Wgsl(
                    include_str!("./wgsl/sqrt_contiguous.wgsl").into(),
                ),
            })
        }
        crate::ArrayType::View(_) => device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Create Shaders Module"),
            source: wgpu::ShaderSource::Wgsl(include_str!("./wgsl/sqrt_view.wgsl").into()),
        }),
    }
}
