...
use crate::{
    ...
    PipelineCompound,
    arr_o_gpu::compute_shaders::{TAN_CONTIGUOUS_SHADERS_PATH, TAN_VIEW_SHADERS_PATH},
    get_stride_from_shape, vector_padding,
};

const PIPELINE_TAN_CONTIGUOUS: &'static str = "pipeline_tan_contiguous";
const PIPELINE_TAN_VIEW: &'static str = "pipeline_tan_view";
...
impl ArrOgpuModule {
        ...
        A: ArrayCompute,
    {
        ...
        let shape_padding: [u32; 8] = vector_padding(shape.clone(), 0, 8)
            .map_err(|err| ArrOgpuErr::Tan(err))?
            .try_into()
            .unwrap();
        ...
        let pipeline = match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => {
                if let Some(pipeline) = read.get(PIPELINE_TAN_CONTIGUOUS) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    let pipeline = set_pipeline(
                        &wgpu.device,
                        &self.common_pipeline_layout,
                        TAN_CONTIGUOUS_SHADERS_PATH,
                    );
                    PipelineCompound::UnsavePipeline(pipeline, PIPELINE_TAN_CONTIGUOUS)
                }
            }
            ArrayType::View(_) => {
                if let Some(pipeline) = read.get(PIPELINE_TAN_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    let pipeline = set_pipeline(
                        &wgpu.device,
                        &self.common_pipeline_layout,
                        TAN_VIEW_SHADERS_PATH,
                    );

                    PipelineCompound::UnsavePipeline(pipeline, PIPELINE_TAN_VIEW)
                }
            }
        };
        ...
        let mut encoder =
            wgpu.device
                .create_command_encoder(&wgpu::wgt::CommandEncoderDescriptor {
                    label: Some("Create Command Encoder For Tan"),
                });
...

        {
            let mut begin_compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("Create Begin Compute Pass For Tan"),
    ...
    A: ArrayCompute,
{
    ...
    let key = match array.check_contiguous_or_view() {
        ArrayType::Contiguous(_) => PIPELINE_TAN_CONTIGUOUS,
        ArrayType::View(_) => PIPELINE_TAN_VIEW,
    };
...
where
    A: ArrayCompute,
{
    let metadata = array.metadata_compound().ok_or(ArrOgpuErr::Tan(
        "Tan Error, Metadata Not Yet Defined For Array A".to_string(),
    ))?;
...
fn set_pipeline(
    ...
    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("Create Pipeline For Tan"),
        layout: Some(layout),
        module: &device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Create Shaders Module For Tan"),
