...
use crate::{
    ...
    PipelineCompound,
    arr_o_gpu::compute_shaders::{COS_CONTIGUOUS_SHADERS_PATH, COS_VIEW_SHADERS_PATH},
    get_stride_from_shape, vector_padding,
};

const PIPELINE_COS_CONTIGUOUS: &'static str = "pipeline_cos_contiguous";
const PIPELINE_COS_VIEW: &'static str = "pipeline_cos_view";
...
impl ArrOgpuModule {
        ...
        A: ArrayCompute,
    {
        ...
        let shape_padding: [u32; 8] = vector_padding(shape.clone(), 0, 8)
            .map_err(|err| ArrOgpuErr::Cos(err))?
            .try_into()
            .unwrap();
        ...
        let pipeline = match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => {
                if let Some(pipeline) = read.get(PIPELINE_COS_CONTIGUOUS) {
                    ...
                    PipelineCompound::UnsavePipeline(pipeline, PIPELINE_COS_CONTIGUOUS)
                }
            }
            ArrayType::View(_) => {
                if let Some(pipeline) = read.get(PIPELINE_COS_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    let pipeline = set_pipeline(
                        &wgpu.device,
                    ...
                    PipelineCompound::UnsavePipeline(pipeline, PIPELINE_COS_VIEW)
                }
            }
        };
        ...
        let mut encoder =
            wgpu.device
                .create_command_encoder(&wgpu::wgt::CommandEncoderDescriptor {
                    label: Some("Create Command Encoder For Cos"),
                });
...

        {
            let mut begin_compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("Create Begin Compute Pass For Cos"),
    ...
    A: ArrayCompute,
{
    ...
    let key = match array.check_contiguous_or_view() {
        ArrayType::Contiguous(_) => PIPELINE_COS_CONTIGUOUS,
        ArrayType::View(_) => PIPELINE_COS_VIEW,
    };
...
where
    A: ArrayCompute,
{
    let metadata = array.metadata_compound().ok_or(ArrOgpuErr::Cos(
        "Cos Error, Metadata Not Yet Defined For Array A".to_string(),
    ))?;
...
fn set_pipeline(
    ...
    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("Create Pipeline For Cos"),
        layout: Some(layout),
        module: &device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Create Shaders Module For Cos"),
