...
use crate::{
    ...
    PipelineCompound,
    arr_o_gpu::compute_shaders::{ABS_CONTIGUOUS_SHADERS_PATH, ABS_VIEW_SHADERS_PATH},
    get_stride_from_shape, vector_padding,
};

const PIPELINE_ABS_CONTIGUOUS: &'static str = "pipeline_abs_contiguous";
...
impl ArrOgpuModule {
        ...
        A: ArrayCompute,
    {
        ...
        let shape_padding: [u32; 8] = vector_padding(shape.clone(), 0, 8)
            .map_err(|err| ArrOgpuErr::Abs(err))?
            .try_into()
            .unwrap();
        ...
        let pipeline = match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => {
                if let Some(pipeline) = read.get(PIPELINE_ABS_CONTIGUOUS) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    let pipeline = set_pipeline(
                        &wgpu.device,
                        &self.common_pipeline_layout,
                        ABS_CONTIGUOUS_SHADERS_PATH,
                    );
                    PipelineCompound::UnsavePipeline(pipeline, PIPELINE_ABS_CONTIGUOUS)
                }
            }
            ArrayType::View(_) => {
                if let Some(pipeline) = read.get(PIPELINE_ABS_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    let pipeline = set_pipeline(
                        &wgpu.device,
                        &self.common_pipeline_layout,
                        ABS_VIEW_SHADERS_PATH,
                    );

                    PipelineCompound::UnsavePipeline(pipeline, PIPELINE_ABS_VIEW)
                }
            }
        };
        ...
        let mut encoder =
            wgpu.device
                .create_command_encoder(&wgpu::wgt::CommandEncoderDescriptor {
                    label: Some("Create Command Encoder For Abs"),
                });
    ...
    A: ArrayCompute,
{
    ...
    let key = match array.check_contiguous_or_view() {
        ArrayType::Contiguous(_) => PIPELINE_ABS_CONTIGUOUS,
        ArrayType::View(_) => PIPELINE_ABS_VIEW,
    };
...
where
    A: ArrayCompute,
{
    let metadata = array.metadata_compound().ok_or(ArrOgpuErr::Abs(
        "Abs Error, Metadata Not Yet Defined For Array A".to_string(),
    ))?;
...
fn set_pipeline(
    ...
    shaders_locate: &'static str,
) -> wgpu::ComputePipeline {
    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("Create Pipeline For Abs"),
        layout: Some(layout),
        module: &device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Create Shaders Module For Abs"),
            source: wgpu::ShaderSource::Wgsl(shaders_locate.into()),
        }),
