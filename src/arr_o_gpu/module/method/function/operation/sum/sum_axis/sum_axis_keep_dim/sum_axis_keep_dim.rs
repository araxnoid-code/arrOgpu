...
impl ArrOgpuModule {
    pub fn sum_axis_keep_dim<A>(&self, array_a: &A, axis: &[u32]) -> Result<GpuArray, ArrOgpuErr>
    where
        A: ArrayCompute,
    {
        ...
        let shader = wgpu.device.create_shader_module(ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("sum_axis_keep_dim.wgsl").into()),
        });
