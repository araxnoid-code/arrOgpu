...
const PIPELINE_MUL_CONTIGUOUS: &'static str = "pipeline_mul_non_scalar_contiguous";
const PIPELINE_MUL_VIEW: &'static str = "pipeline_mul_non_scalar_view";
        ...
        B: ArrayCompute + ?Sized,
    {
        ...
        let pipeline = match (
            ...
            (ArrayType::Contiguous(_), ArrayType::Contiguous(_)) => {
                if let Some(pipeline) = read.get(PIPELINE_MUL_CONTIGUOUS) {
                    ...
                    PipelineCompound::UnsavePipeline(
                        ...
                        PIPELINE_MUL_CONTIGUOUS,
                    )
                }
            }

            _ => {
                if let Some(pipeline) = read.get(PIPELINE_MUL_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    drop(read);
                    PipelineCompound::UnsavePipeline(
