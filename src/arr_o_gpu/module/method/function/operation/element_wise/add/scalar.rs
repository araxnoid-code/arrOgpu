...
const PIPELINE_SCALAR_ADD_CONTIGUOUS: &'static str = "pipeline_add_scalar_contiguous";
const PIPELINE_SCALAR_ADD_VIEW: &'static str = "pipeline_add_scalar_view";
        ...
        A: ArrayCompute,
    {
        ...
        let pipeline = match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => {
                if let Some(pipeline) = read.get(PIPELINE_SCALAR_ADD_CONTIGUOUS) {
                    ...
                    PipelineCompound::UnsavePipeline(
                        ...
                        PIPELINE_SCALAR_ADD_CONTIGUOUS,
                    )
                }
            }
            ArrayType::View(_) => {
                if let Some(pipeline) = read.get(PIPELINE_SCALAR_ADD_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    drop(read);
                    let pipeline_layout = &self.common_pipeline_layout;
