...
const PIPELINE_ADD_CONTIGUOUS: &'static str = "pipeline_add_non_scalar_contiguous";
const PIPELINE_ADD_VIEW: &'static str = "pipeline_add_non_scalar_view";
        ...
        B: ArrayCompute + ?Sized,
    {
        ...
        let pipeline = match (
            ...
            (ArrayType::Contiguous(_), ArrayType::Contiguous(_)) => {
                if let Some(pipeline) = read.get(PIPELINE_ADD_CONTIGUOUS) {
                    ...
                    PipelineCompound::UnsavePipeline(
                        create_pipeline(
                            ...
                            self.function_execute_opt.add.compute_workgroup_size_x,
                        ),
                        PIPELINE_ADD_CONTIGUOUS,
                    )
                }
            }

            _ => {
                if let Some(pipeline) = read.get(PIPELINE_ADD_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    drop(read);
                    PipelineCompound::UnsavePipeline(
...

        {
            ...
            let workgroup_size_x = self.function_execute_opt.add.compute_workgroup_size_x;
