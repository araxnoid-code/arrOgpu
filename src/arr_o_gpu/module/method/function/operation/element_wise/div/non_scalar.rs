        ...
        B: ArrayCompute + ?Sized,
    {
        ...
        let pipeline = match (
            ...
            (ArrayType::Contiguous(_), ArrayType::Contiguous(_)) => {
                if let Some(pipeline) = read.get("pipeline_div_non_scalar_contiguous") {
                    ...
                    PipelineCompound::UnsavePipeline(
                        ...
                        "pipeline_div_non_scalar_contiguous",
                    )
                }
            }

            _ => {
                if let Some(pipeline) = read.get("pipeline_div_non_scalar_view") {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    drop(read);
                    PipelineCompound::UnsavePipeline(
                        set_pipeline(&wgpu.device, array_a, array_b, &self.common_pipeline_layout),
                        "pipeline_div_non_scalar_view",
                    )
                }
            }
        };
    ...
    B: ArrayCompute + ?Sized,
{
    ...
    match (
        ...
        (ArrayType::Contiguous(_), ArrayType::Contiguous(_)) => {
            write.insert("pipeline_div_non_scalar_contiguous", pipeline);
        }
        _ => {
            write.insert("pipeline_div_non_scalar_view", pipeline);
        }
    };
}
