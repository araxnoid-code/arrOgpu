...
use crate::{
    ...
    arr_o_gpu::{
        compute_shaders::{DIV_SCALAR_CONTIGUOUS_SHADERS_PATH, DIV_SCALAR_VIEW_SHADERS_PATH},
...
const PIPELINE_SCALAR_DIV_CONTIGUOUS: &'static str = "pipeline_div_scalar_contiguous";
const PIPELINE_SCALAR_DIV_VIEW: &'static str = "pipeline_div_scalar_view";

impl ArrOgpuModule {
    pub(crate) fn div_skalar<A>(
        &self,
        array: &A,
        ...
        A: ArrayCompute,
    {
        ...
        let pipeline = match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => {
                if let Some(pipeline) = read.get(PIPELINE_SCALAR_DIV_CONTIGUOUS) {
                    ...
                    PipelineCompound::UnsavePipeline(
                        ...
                        PIPELINE_SCALAR_DIV_CONTIGUOUS,
                    )
                }
            }
            ArrayType::View(_) => {
                if let Some(pipeline) = read.get(PIPELINE_SCALAR_DIV_VIEW) {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    drop(read);
                    let pipeline_layout = &self.common_pipeline_layout;
...

        {
            let mut begin_compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("Create Begin Compute Pass For Div"),
    ...
    A: ArrayCompute,
{
    ...
    let array_a_metadata_buffer = array.metadata_compound().ok_or(ArrOgpuErr::Div(
    ...
    let buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Create Static Interface Buffer For Div"),
    ...
    A: ArrayCompute,
{
    device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => DIV_SCALAR_CONTIGUOUS_SHADERS_PATH.into(),
            ArrayType::View(_) => DIV_SCALAR_VIEW_SHADERS_PATH.into(),
        }),
    })
}
