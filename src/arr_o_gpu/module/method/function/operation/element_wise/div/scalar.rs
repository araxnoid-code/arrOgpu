        ...
        A: ArrayCompute,
    {
        ...
        let pipeline = match array.check_contiguous_or_view() {
            ArrayType::Contiguous(_) => {
                if let Some(pipeline) = read.get("pipeline_div_scalar_contiguous") {
                    ...
                    PipelineCompound::UnsavePipeline(
                        ...
                        "pipeline_div_scalar_contiguous",
                    )
                }
            }
            ArrayType::View(_) => {
                if let Some(pipeline) = read.get("pipeline_div_scalar_view") {
                    PipelineCompound::PipelineCache(pipeline)
                } else {
                    drop(read);
                    let pipeline_layout = &self.common_pipeline_layout;
    ...
    A: ArrayCompute,
{
    ...
    let key = match array.check_contiguous_or_view() {
        ArrayType::Contiguous(_) => "pipeline_div_scalar_contiguous",
        ArrayType::View(_) => "pipeline_div_scalar_view",
    };
