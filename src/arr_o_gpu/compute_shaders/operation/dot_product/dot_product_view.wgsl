    ...
    if local_id.x < used_thread{
        ...
        if reduction_counter.value == 0{
            if end - work.x * 512 < reduction_len{
                let a = heap[execute_args[0].pointer.x + pointing_a(start)] * heap[execute_args[1].pointer.x + pointing_b(start)];
                let b = heap[execute_args[0].pointer.x + pointing_a(end)] * heap[execute_args[1].pointer.x + pointing_b(end)];
